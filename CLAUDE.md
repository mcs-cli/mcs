...
- **Non-interactive prompt resolution**: when stdin is not a TTY, every prompt resolves as seeded/stored value → declared default (a `select` default must be an option, a multi-match `fileDetect` default must be a match; an explicit `""` counts) → failure. `Configurator.configure` runs `CrossPackPromptResolver.resolveNonInteractively` before any removal or install, throws `PromptResolutionError` listing every `pack: key` (never values), and otherwise seeds its answers so no reader runs; the executor and shared-prompt resolver keep the same rule as a backstop, and piped stdin is ignored rather than read. `--dry-run` only warns; `mcs update` skips a failing scope, finishes the rest, then exits non-zero
