...
/// (`NCCS` is 20 on Darwin and 32 on Glibc; `VMIN` is index 16 there and 6 here), so the index has
/// to come from the platform's own constant rather than a literal. `tcflag_t` likewise differs —
/// `UInt` on Darwin, `UInt32` on Glibc — so every flag mask is widened through `tcflag_t(...)`.
